...
function AlokasiRuangan({ programStudiData, ruangKelasData, initialAlokasiData = [] }) {
    ...
    return (
                        ...
                        <>
                            <div className="mb-4">
                                ...
                                >
                                    ...
                                    {['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K'].map(gedung => (
                                        <option key={gedung} value={gedung}>Gedung {gedung}</option>
                                    ))}
                                </select>
                            </div>
                    ...
                    <table className="min-w-full table-auto bg-white border border-gray-300 shadow-md text-sm">
                        <thead>
                            <tr className="bg-gray-100 text-gray-600 uppercase leading-normal">
                                ...
                                <th className="py-2 px-2 border border-gray-300 text-center">Program Studi</th>
                                <th className="py-2 px-2 border border-gray-300 text-center">Ruangan</th>
                                <th className="py-2 px-2 border border-gray-300 text-center">Status</th>
        ...
        const programStudi = programStudiData.find(
            (studi) => studi.id === alokasi.program_studi_id
        );

        // Cari nama ruangan berdasarkan ID
        const ruangKelas = alokasi.ruang_kelas_ids.map((ruangId) => {
            const ruang = ruangKelasData.find((ruangKelas) => ruangKelas.id === ruangId);
            return ruang ? ruang.nama_ruang : 'Tidak Ditemukan';
        });

        return (
            <tr key={alokasi.id} className="border-b border-gray-300 hover:bg-gray-50">
                <td className="py-2 px-2 border border-gray-300 text-center">{alokasi.id}</td>
                <td className="py-2 px-2 border border-gray-300 text-center">
                    {programStudi ? alokasi.programStudi.nama_program_studi : 'Tidak Ditemukan'}
                </td>
                <td className="py-2 px-2 border border-gray-300 text-center">
                    {alokasi.ruangKelas.join(', ')}
                </td>
                <td className="py-2 px-2 border border-gray-300 text-center">{alokasi.status}</td>
                <td className="py-2 px-2 border border-gray-300 text-center">
                    {alokasi.status === 'pending' ? (
                        <>
                            <button
                                onClick={() => handleSend(alokasi.id)}
                                className="bg-yellow-500 text-white font-semibold py-1 px-2 rounded-lg mr-2 hover:bg-yellow-600 transition duration-300"
                            >
                                Kirim
                            </button>
                            <button
                                onClick={() => setEditId(alokasi.id)}
                        ...
                        <span className="text-gray-500 italic">
                            {alokasi.status === 'onprocess' ? 'On Process' : 'Approved'}
                        </span>
                    )}
                </td>
            </tr>
